...
const BASE_FILES = ['reset.css', 'icon-glyph.css'];
...
const COMPONENT_JS_FILES = ['cards.js', 'tags.js', 'carousel.js', 'tabs.js'];
...
const js = readFiles('src/components', COMPONENT_JS_FILES);
...
// links every component file individually from src/ (so edits show up
// on refresh with no build step) — that's what this inlines, using the
// same compiled CSS/JS dist/csa-library.html already builds above.
...
let previewHtml = readFileSync(join(__dirname, 'src/docs/index.html'), 'utf8')
  ...
  .replace(/(<script src="\.\.\/components\/[a-z]+\.js"><\/script>\n?)+/, `<script>\n${js}\n</script>\n`);
